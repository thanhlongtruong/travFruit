...
router.post("/google-sheet", async (req, res) => {
    ...
    const newestDonhang = await DonHang.findOne({userId: _id}).sort({createdAt: -1});
    if(!donhang) {
    ...
    if(newestDonhang){
        const expiresAt = new Date(newestDonhang?.createdAt?.getTime() + 15 * 60 * 1000);
        if(currentTime > expiresAt && (newestDonhang.expiredAt !== null || newestDonhang.expiredAt !== undefined || newestDonhang.expiredAt !== "null")) {
            AMOUNT_VIETQR = parseInt(formatNumber(newestDonhang.tongGia.split(" ")[0]));
            DESCRIPTION_VIETQR = `${convertDateToVNDate(newestDonhang?.expiredAt)} ${newestDonhang._id}`.replace(/[:/]/g, "");
            if((response?.data.data[0]["Mô tả"].slice(-40)) === DESCRIPTION_VIETQR && AMOUNT_VIETQR === response?.data.data[0]["Giá trị"]) {
    ...
    DESCRIPTION_VIETQR = `${convertDateToVNDate(donhang?.expiredAt)} ${donhang._id}`.replace(/[:/]/g, "");
    console.log(response?.data.data[0]["Mô tả"].slice(-40), DESCRIPTION_VIETQR, (response?.data.data[0]["Mô tả"].slice(-40)).includes(DESCRIPTION_VIETQR), AMOUNT_VIETQR === response?.data.data[0]["Giá trị"]);
    if((response?.data.data[0]["Mô tả"].slice(-40)) === DESCRIPTION_VIETQR && AMOUNT_VIETQR === response?.data.data[0]["Giá trị"]) {
      return res.status(200).json({
        message: "SUCCESS",
        data: {
